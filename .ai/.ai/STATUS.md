...
- **TOTAL removal of data localStorage (API-only, 100% DB sync)**: Removed ALL reads/writes of `va_about`, `va_experiences`, `va_projects` (and `lsGet`/`lsSet`/`lsRemove`/`LS_KEYS` helpers) from `script.js` and `project-form.js`. Every public load (`loadPublicProfile`, `loadPublicExperiences`, `loadPublicProjects`, `renderManageList`) and every admin edit/lightbox fetch now uses `fetch(..., { cache:'no-store', headers:{ Pragma, Cache-Control:'no-cache, no-store' } })` — Mobile & Desktop render straight from Supabase. `localStorage.clear()` runs after every Save/Update/Delete (about, experience, project) in both `.then` and `.catch`. `index.html` added inline `<script>window.localStorage.clear();</script>` (line 158) before `script.js` so every page load starts clean. Only auth (`isLoggedIn`, `adminToken`) + theme (`portfolio_theme`) keys remain in localStorage; login.html intentionally does NOT clear
